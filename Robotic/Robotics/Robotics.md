...
* 首尾的坐标系定义
  * Frame $\left\{0\right\}$ 和 Frame $\left\{1\right\}$ 相同
  * $\left\{n\right\}$ 的 $\hat{X}_i$ 取和 $\hat{X}_{i-1}$ 同方向
...
$$
^0\boldsymbol{v}=\left[\begin{matrix}^0v\\^0\omega\end{matrix}\right]=^0J(\Theta)\dot{\Theta}
$$
