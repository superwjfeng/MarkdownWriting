...
模型量化是一种深度学习模型优化技术，通过将模型的权重和激活从较高精度（如32位浮点数）转换为较低精度（如16位浮点数、8位整数等），以减少模型的存储需求、提高推理速度、降低能耗。这种技术对于资源受限的设备（如移动设备、嵌入式系统）特别有价值
...
[10.3. 模型压缩 — 机器学习系统：设计和实现 1.0.0 documentation](https://openmlsys.github.io/chapter_model_deployment/model_compression.html?highlight=量化#id5)
