...
1. 源端注入的速率不超过瓶颈链路带宽
2. BDP=RTprop* BtlBW，从源端注入的等待被确认的inflight数据不超过BDP
3. 与水管不同，通信是经常变化的，需经常测量2个量，按照以上思路进行控制
...
ssh-add 这个命令不是用来永久性的记住使用的私钥的。实际上，它的作用只是把指定的私钥添加到 ssh-agent 所管理的一个 session 当中。而 ssh-agent 是一个用于存储私钥的临时性的 session 服务，也就是说重启shell之后，ssh-agent 服务也就重置了
...
用 `-v, -vv, -vvv` 来打印debug信息 

当使用 Git 进行 push 操作时，Git 需要访问本地私钥以进行身份验证。如果 Git 找不到私钥，则可能是以下几个原因导致的：

1. 私钥文件不存在或未设置正确的权限：确保的私钥文件存在于正确的位置，并且具有正确的权限。在大多数情况下，私钥应该位于 ~/.ssh 目录下，并且应该具有 600 的权限（只能由所有者读取和写入）

2. 私钥文件名称不正确：如果私钥文件名称不是默认名称（id_rsa），则需要将其指定为 Git 的配置中。可以在 Git Bash 中运行以下命令来设置：
...
3. SSH 代理没有正确设置：如果正在使用 SSH 代理，可能需要配置代理以使 Git 可以找到私钥。可以在终端中运行以下命令设置代理
