...
### 编译器 Complier vs. 解释器 Interpreter

python和C++都是面向对象的，但他们的区别也很明显

* python是脚本语言，脚本语言不需要编译，直接由解释器逐行解释运行
* C++是编译型语言，需要通过编译器编译为二进制码可执行文件执行

可以说python不是一门语言，python解释器只是一个用C写的大型工程软件，用来对我们编写的python代码进行自动化分析
