...
CAS, Compare And Swap：假设内存中存在一个变量i，它在内存中对应的值是A（第一次读取），此时经过计算之后，要把它更新成B，那么在更新之前会再读取一下i现在的值C，若在业务处理的过程中i的值并没有发生变化，即A和C相同，才会把i更新/交换为新值B。如果A和C不相同，那说明在计算时，i的值发生了变化，则不更新/交换成B。最后，CPU会将旧的数值返回。而上述的一系列操作由CPU指令来保证是原子的
