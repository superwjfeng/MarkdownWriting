...
* Anti dependence 和 Output dependence 是 false dependence，因为没有语义上的依赖，只是因为寄存器标号不是无限多造成的name dependency 
  ...
  * Output dependence
    ...
    如果交换第一步和第三部的顺序，则会影响到第二步的结果，所以这两步无法在流水线上并行执行。但是如果将其中任意一个r3换成另一个寄存器就可以消除output dependence了
...
另一方面为了进行静态调度，编译器要知道很多信息，但是不幸的是，任何在运行时才能知道的信息编译器都是无法知晓的，比如说依赖于长度的操作的延迟（比方说如果乘法有一个操作数在运行时确定为0，那么就能极大的缩短操作时间）、内存地址、分支方向等
...
超长指令字 Very Long Instruction Word VLIW ISA：一种类型的ISA，支持在单个指令中使用不同的编码位来定义多个可同时被发射的独立操作

将VLIW的思路套到静态多发射上：可以将发射指令包看成是一条需要进行多种操作的大指令，同时也要对同一周期发射的指令类型做出限制

### 动态多发射处理器

动态多发射处理器也称为**超标量处理器 superscalar processors**，或者直接叫superscalars。超标量是一种高级流水线技术，指处理器能够在动态执行时选择指令，并在同一个cycle执行一条以上的指令。同一个周期最多同时执行N条指令的被称为N-wide superscalar
...
* 每个线程块有共享内存 Shared Memory
* 所有的线程都可以访问全局内存 Global Memory，全局内存就是显存
...
每种内存都有自己的作用域，生命周期和缓存行为。CUDA中每个线程都有自己的私有的本地内存，寄存器
线程块有自己的共享内存，对线程块内所有线程可见
花有缝費数酵访问读取常最内存和纹理内存，但是不能写，因內
金农乒全篙携容在聚籃黑容麻菱星怎吞署箱馬綉生荔南兩合區
