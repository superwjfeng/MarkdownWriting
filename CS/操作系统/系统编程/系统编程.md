...
Ubuntu 中，软件源的管理和配置主要通过编辑配置文件进行。软件源的列表存储在 `/etc/apt/` 目录下的一些特定文件中

### 配置文件

1. `/etc/apt/sources.list`：这是主要的配置文件，包含了默认的软件源的列表。每个条目都定义了一个软件源，并且有一个格式，通常看起来像这样：
   ...
   - `focal` 是 Ubuntu 发行版的代号
   - `main` 和 `restricted` 是该仓库中包含的分区。有哪些分区见下

2. `/etc/apt/sources.list.d/`：这个目录包含了由特定应用程序或第三方添加的软件源列表文件。比如，当添加一个 Personal Package Archive (PPA) 或某些软件（例如 CUDA）时，就会在这个目录下创建 `.list` 文件

3. `trusted.gpg` 和 `trusted.gpg.d/`：这些文件和目录包含了认证密钥，APT 使用这些密钥来验证从软件源下载的包的完整性和真实性。如果软件源的 GPG 密钥不在这里，那么它的软件包将无法被认证和安装

### 软件分区

Ubuntu 的软件源通常由以下几类组成：

1. main（主要仓库）：这个存储库包含了由 Ubuntu 团队维护的自由且开放源代码的软件。这些软件都受到安全更新和支持的保障
2. universe（宇宙仓库）：universe 存储库包含由社区维护的自由和开放源代码软件。这些软件不由 Ubuntu 官方团队提供安全更新和支持
3. restricted（限制仓库）：限制存储库包含一些非自由软件，这些软件通常是因为版权或法律原因无法包含在 main 存储库中。尽管如此，这些软件仍然提供了某些重要功能，例如专有驱动程序
4. multiverse（多元宇宙仓库）：multiverse 存储库包含的软件可能不是自由或开源的。这里包括了一些受版权或法律限制的软件包，比如一些视频编解码器和游戏
5. backports（回溯仓库）：backports 存储库允许用户安装较新版本的软件包，这些软件包已经被回溯到当前稳定发行版。这样用户可以使用到最新的软件而无需升级整个操作系统
6. security（安全仓库）：安全存储库包含了安全相关的更新。这是用来修复已知漏洞的更新，以确保系统安全
7. updates（更新仓库）：更新存储库提供了除安全更新之外的所有官方支持更新，这可能包括错误修复、改进和翻译更新等
8. proposed（建议仓库）：建议存储库包含即将发布到 'updates' 存储库中的更新。这些软件包用于测试目的，可能尚未完全测试
9. partner（合作伙伴仓库）：合作伙伴存储库包含一些第三方应用程序，这些应用是由 Ubuntu 的合作伙伴提供，并由他们负责维护和更新
...
`fwrite()` 相比于 `write()` 在每次写入的块比较小的时候是有优势的，因为 `fwrite()` 会把写的东西攒在用户级缓冲区中，等到了一定的量在统一往下送。但是如果块比较大的时候反而是 `write()` 性能好，因为这时候 `fwrite()` 要拷贝两层的缓冲区
...
* `&` 符可以让进程在后台运行，shell 会打印出一个作业编号和PID

  如果使用 `&` 把任务放到了后台并且没有重定向输出，任务的标准输出和错误通常仍然会显示在终端上。但这可能会干扰到你当前的工作
...
注意：当通过 SSH 连接到一个远程服务器并且使用 `&` 在后台启动一个任务时，该任务会与 SSH 会话关联起来。如果关闭了 SSH 连接或者连接因为某种原因被断开（例如网络中断或超时），那么默认情况下，所有与该会话关联的后台任务都会收到 `SIGHUP` 信号并被终止。上面的所有命令没法解决这个问题。下面给出解决的方法

* 使用 `nohup`

  `nohup` 命令可以阻止后台任务在退出登录会话后接收到 `SIGHUP` 信号

  ```cmd
  $ nohup your_command &
  ```

  这个命令会使得 `your_command` 在断开 SSH 连接后继续运行，并且默认情况下把输出重定向到 `nohup.out` 文件中，除非指定了另一个文件

* 使用 `disown`

  如果已经在后台启动了一个任务，然后意识到我们需要断开连接，可以使用 `disown` 命令来移除特定作业对 `SIGHUP` 的响应。首先，通过 `jobs` 查找后台作业的列表，然后使用 `bg` 将作业置于后台执行（如果它还未在执行），最后用 `disown` 来从 shell 的作业表中移除它：

  ```cmd
  $ jobs      # 查看所有作业
  $ bg %1     # 如果作业不在运行，将其置于后台执行 (假设作业编号是 1)
  $ disown %1 # 移除作业 (假设作业编号是 1)
  ```
...
不同优先级的进程之间可以抢占，相同优先级之间的根据调度策略有所不同
...
* flags：标志位，用于指定创建新进程的方式。下面只介绍与用于创建线程有关的flags，其他具体的flags可以看书。其中有一组非常重要的 `CLONE_NEW*` 系列的flags用于创建新的命名空间，这是docker的重要基础，这部分可以在 *Cloud.md* - Docker技术基石中查看
...
* 调用线程将被阻塞，直到被等待的线程退出。**不能以非阻塞 nonblocking 方式进行join，使用条件变量可以实现类似的功能***
...
条件变量的接口和锁的接口非常相似，因为它们都遵守 POSIX 统一设计标准
...
条件变量的主要操作是发送信号 signal 和等待 wait。注意：**条件变量必须要和 mutex 互斥锁一块使用**
...
条件变量从使用、进入阻塞等待到唤醒的过程中会发生这些事情

1. 调用 pthread_cond_wait 前需要先手动对互斥量 mutex 上锁，才能把 &mutex 传入 pthread_cond_wait 函数
2. pthread_cond_wait 内部会自动解锁互斥量 mutex
3. pthread_cond_wait 堵塞调用线程，直至另一线程用 pthread_cond_signal 或 broadcast 就条件变量 cond 发出信号唤醒
4. pthread_cond_wait 收到信号醒来，然后内部在返回前自动重新锁定 mutex
5. 处理完等待的任务之后，需要手动解锁

也就是说 pthread_cond_wait 的使用流程为下，以 thread_pool 等待任务队列不为空为例

```c
while (true) {
    pthread_mutex_lock(tp->mtx_);              // 线程池的任务队列上锁
    while (tp->task_queue_->empty()) {         // 任务队列为空的时候进入循环等待条件变量唤醒
        pthread_cond_wait(tp->cond_, tp->mtx_); 
    }
    t = tp->task_queue_->front();              // 从任务队列中取任务
    tp->task_queue_->pop(); 
    pthread_mutex_unlock(tp->mtx_);            // 线程池的任务队列解锁

    t->callback();                             // 任务句柄回调
}
```

<img src="pthread条件变量与锁的关系.drawio.png" width="80%">

结合上面的图，我们可以回答三个核心问题

1. 为什么调用 pthread_cond_wait 之前要手动上锁？在线程条件判断成功进入 while 到调用 pthread_cond_wait 之间有一段空窗期，在多线程环境下此时如果不加锁，可能会有其他线程也判断条件满足然后提前 A 调用 pthread_cond_wait 并被唤醒执行任务，这样要么 A 永远不会被唤醒，要么被不是 A 所判断的条件唤醒

2. 为什么进入 pthread_cond_wait 之后内部会自动解锁？因为要让条件能够被改变，比如说上面线程池使用同一把锁来保护任务队列这个临界资源的，线程池必须要拿到锁才能更改任务队列

3. 为什么被唤醒后在退出 pthread_cond_wait 之前又会自动上锁？

   考虑这样一种情况：当进入 pthread_cond_wait 之后会自动解锁，如果此时任务迟迟不到来，会有多个线程分别抢到锁然后全部进入 pthread_cond_wait，这样 pthread_cond_signal 唤醒到来的时候，所有线程都会被唤醒

   因此在 pthread_cond_wait 退出之前会自动上锁，抢到锁的线程会去执行它的任务，其他任务则等待锁资源

   自动上锁的目的在于要保证抢到锁的线程会把判断条件重新置为不成立，防止没有抢到锁的线程在抢到锁会是虚假唤醒（也就是说明明任务已经被其他线程抢走了，判断条件却还是成立的）。此时判断条件不成立，没有抢到任务的被唤醒进程就会再次进入 pthraed_cond_wait 等待 signal 的唤醒
...
其中一个非常容易错的点是，当多个线程批量退出的时候，必须要在 pthread_cond_wait 之前和之后分别加锁和解锁，因为是多个线程 broadcast 同时被释放，但只有一个线程能竞争到锁，此时当这个线程退出时，它并没有释放锁，那么其他线程就必须要阻塞等待，也就无法退出了
...
堆向上增长的边界被称为 program break，所以 brk 和 sbrk 顾名思义就是通过调整 program break 来增大或缩小堆区，从而分配堆内存区

* 移动到绝对地址：`brk()` 函数直接修改有效访问范围的末尾地址 addr 实现分配与回收

* 移动相对大小：调用 `sbrk()` 将 program break 在原有地址上增加/减少从参数 increment 传入的大小（单位是 byte）。sbrk 就是复用了 brk
...
alloca 函数用于在栈上动态分配内存空间。它的作用是在当前函数的栈帧上分配一段内存空间，当函数返回时，该内存空间自动释放

使用 `alloca()` 分配的内存空间通常不需要手动释放，因为它们随着函数的返回而被自动释放
