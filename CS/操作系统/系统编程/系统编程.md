...
2. pthread_cond_wait 会解锁互斥量 mutex
3. pthread_cond_wait 堵塞调用线程，直至另一线程就条件变量 cond 发出信号
4. pthread_cond_wait 收到信号醒来，然后在返回前重新锁定 mutex

首先考虑一个问题，如果不使用信号量该怎么判断一个临界资源已经就位？
