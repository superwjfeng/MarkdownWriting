...
1. 调用pthread_cond_wait前需要先手动对互斥量mutex上锁，才能把&mutex传入pthread_cond_wait函数
...
1. 为什么调用 pthread_cond_wait 之前要手动上锁？在线程条件判断成功进入 while 到调用 pthread_cond_wait 之间有一段空窗期，在多线程环境下此时如果不加锁，可能会有其他线程也判断条件满足然后提前A调用 pthread_cond_wait 并被唤醒执行任务，这样要么A永远不会被唤醒，要么被不是A所判断的条件唤醒
