...
`/proc/cpuinfo` 文件包含了有关系统中每个CPU的信息。以一台云服务的机器为例，以下是一些常见字段
...
* 系统服务管理工具 systemd：大多数现代Linux发行版使用systemd作为初始化系统和服务管理器，它负责初始化系统，管理系统进程，并提供一种简单而强大的方法来管理和控制系统服务。`systemd` 的设计目标包括启动速度的优化、并行性、模块化和更好的日志记录
...
1. 为什么调用pthread_cond_wait之前要手动上锁？在线程条件判断成功进入 while 到调用 pthread_cond_wait 之间有一段空窗期，在多线程环境下此时如果不加锁，可能会有其他线程也判断条件满足然后提前A调用 pthread_cond_wait 并被唤醒执行任务，这样要么A永远不会被唤醒，要么被不是A所判断的条件唤醒

2. 为什么进入pthread_cond_wait之后内部会自动解锁？因为要让条件能够被改变，比如说上面线程池使用同一把锁来保护任务队列这个临界资源的，线程池必须要拿到锁才能更改任务队列

3. 为什么被唤醒后在退出pthread_cond_wait之前又会自动上锁？

   考虑这样一种情况：当进入pthread_cond_wait之后会自动解锁，如果此时任务迟迟不到来，会有多个线程分别抢到锁然后全部进入 pthread_cond_wait，这样pthread_cond_signal 唤醒到来的时候，所有线程都会被唤醒
