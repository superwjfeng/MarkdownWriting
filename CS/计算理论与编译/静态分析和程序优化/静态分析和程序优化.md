...
- 类似地我们也能定义出最大下界（Greatest Lower Bound, GLB or **meet**）为 $\sqcap S$
...
Monotonicity 单调性：如果 $x \sqsubseteq y \Rightarrow f(x)\sqsubseteq f(y)$，则说函数 $f: L \rightarrow L$ 是**单调的**
...
* 可能性分析 May Analysis：输出的结论**可能**为 true，即做的是 over-approximation，此时 safe == over
...
* BB 之间的控制流
  ...
  * 正向
    $$
    OUT[B]=f_B(IN[B]),\ f_B=f_{s_n}\circ\dots\circ f_{s_2}\circ f_{s_1}\\ IN[B]=\bigwedge_{P\ a\ predecessor\ of\ B}{OUT[P]}
    $$

  * 反向
    $$
    IN[B]=f_B(OUT[B]),\ f_B=f_{s_1}\circ\dots\circ f_{s_{n-1}}\circ f_{n}\\ OUT[B]=\bigwedge_{P\ a\ successor\ of\ B}{IN[P]}
    $$
...
* Transfer function
  $$
  OUT[B]=\bigcup_{S\ a\ successor\ of\ B}{IN[S]}\\IN[B]=use_B\cup(OUT[B]-def_B)
  $$
...
x op y 在 p 点可用的条件：从流图入口结点到达 p 的每条路径都对 x op y 求了值，且在最后一次求值之后再没有对 x 或 y 赋值
...
常量传播分析 constant propagation analysis 通过识别并替换程序中不变的变量，提高程序性能。该分析发现并传播程序中可以在编译时固定的常量
