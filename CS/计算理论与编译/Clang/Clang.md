...
>
> 通过封装 LLVM 对 OS 系统调用接口的使用，我们使得 LLVM 工具链和运行时库能够更容易地移植到新平台，**因为（理论上）只有 `lib/Support` 需要被移植**。这个库还减少了 LLVM 其他部分对于 `#ifdef` 使用和特定操作系统的特殊情况的混乱。这样的使用被替换成对 `include/llvm/Support` 提供的接口的简单调用。
>
...
`clangd` 是一个基于 LLVM/Clang 的 **语言服务器协议（Language Server Protocol, LSP）** 实现，专门为 C、C++ 和 Objective-C 等语言提供代码编辑器的智能支持。它通过 LSP 与编辑器（如 VS Code、Vim、Emacs 等）通信，提供代码补全、语法高亮、错误检查、跳转到定义、查找引用、重构等功能
...
可以使用 `.clang-format-ignore` 来添加不需要 format 的文件或目录
