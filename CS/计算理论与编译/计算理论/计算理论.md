...
设a、b和c是 $\Gamma$ 中的符号，u和v是 $\Gamma^*$ 中的字符串，$q_i$ 和 $q_j$ 是状态，则 $uaq_ibv$ 和 $uq_jacv$ 是两个格局，若转移函数满足 $\delta(q_i,b)=(q_j,c,L)$，则称 $uaq_ibv$ yields $uq_jacv$。前面的是图灵机左移的情况，右移的情况同理

当读写头处于格局的两个端点之一时，会产生特殊变化。

M在输入w上的起始格局 start configuration





若一个语言能够被某一个图灵机识别，则称该语言是图灵可识别的 Turing-recognizable
...
若一个语言能够被某一个图灵机判定，则称该语言是图灵可判定的 Turing-decidbale，简称可判定的
...
Diophantine equations 就是一个根为整数的不定式方程

### Church-Turing论断

Church-Turing Thesis, Church-Turing 论断
