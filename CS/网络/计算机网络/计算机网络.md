...
URL Uniform Resource Locator, URL 统一资源定位符，俗称网址，它的完整格式为下面这个字符串。其中[访问凭证信息]（支持匿名访问）、[端口号]、[查询]、[片段ID]都属于选填项
...
域名在使用的时候必须被转换为 IP 地址，访问网络服务，服务端必须具有 Port 端口号。特别是端口号，但使用确定协议的时候，一般会缺省端口号，但浏览器等会自动添加。这是因为**应用层协议规定了强绑定关系**，即 httpServer 对应80号端口、httpsServer 对应443端口、sshd 对应 22 号端口，这就像是 110 对应警察，119 对应火警。一般来说用户能使用的只有 1024 之后的端口，之前的端口都被协议占用了

因为网络资源都是部署在 Linux 服务器上的，所以**URL实际上就是服务器上文件资源的地址**

### URL 中出现的字符的意义

* `?` - 查询字符串（Query string）的开始：用于标示URL的查询部分的开始。查询字符串通常包含一系列**键值对**，每个键值对提供了额外的参数来指定请求或配置资源
* `&` - 查询字符串内多个键值对的分隔符：在查询字符串中用于分隔不同的键值对
* `=` - 查询字符串中键与值的分隔符：在查询字符串中用于分隔键和相应的值
* `#` - 片段标识符（Fragment identifier）: 指向URL内的一个锚点，即页面上某个特定的位置。浏览器会滚动到带有相应ID的元素处
* `/` - 路径分隔符：在 URL 中用于分隔路径组件，如目录和文件名
* `:` - 协议分隔符：分隔 URL 的协议部分和其余部分，如 `http://`
* `%` - 百分号编码：用于编码 URL 中的特殊字符。由于 URL 仅允许一小部分字符直接出现在其中，其他字符需要通过一种编码转换成有效的格式，这就是通过百分号加两位十六进制数来表示字符的 ASCII 码值
* `+` 或 `%20` - 空格的替代表示：URL中不能直接包含空格，因此使用 `+` 或 `%20` 来表示
* `-`, `_`, `.`, `!`, `*`, `'`, `(`, `)` - 保留字符：这些字符在URL中有特殊用途但也可用于表示它们自身，它们通常不必进行编码
* `;` - 参数分隔符：有时用作另外一种方式来分隔URL中的参数，尽管这种用法并不像 `&` 那样普遍
...
<img src="HTTP请求格式.drawio.png" width="80%">
...
* 首部行 header line
  ...
  * Connection：进行请求的连接方式，用于指定客户端与服务器之间的连接管理方式。它可以影响连接的生命周期和持久性
    * Keep-Alive：表示客户端希望保持与服务器的持久连接/长连接 persistent connection/HTTP connection reuse，以便在同一连接上发送多个请求和接收多个响应。这样可以减少连接的建立和关闭的开销，提高性能。**这是HTTP/1.1的默认Connection方式**。注意⚠️：不要和 TCP 协议的 keep-alive 保活机制弄混了
...
<img src="HTTP响应格式.drawio.png" width="80%">
...
* 首部行 header line/头字段 header field
  ...
  * Content-Encoding 和 Accept-Encoding：分别说明Server返回数据使用了什么压缩方法以及Client可以接受哪些压缩方法，比如gzip、deflate等（无损压缩）
...
HTTP/1.1 引入了请求的 pipeline 技术。顾名思义，和 CPU 的流水线一样，HTTP 的 pipeline允许客户端在不等待响应的情况下连续发送多个请求。这样服务器在收到请求后，可以并行处理多个请求，并将响应按照请求的顺序返回给客户端。通过这种方式，可以充分利用网络带宽和减少通信的延迟，提高性能

但是这个 pipeline 是默认关闭的，理由就是因为请求的队头阻塞 Head-of-Line Blocking 问题：如果某个请求在处理过程中遇到阻塞或错误，那么在它之后的所有请求都会被阻塞，直到问题解决或超时

另外一个很容易想到的问题是，网络毕竟不是 CPU，CPU 的流水线只需要考虑依赖，HTTP 的问题是，网络传播的延迟是不可预计的，如果早发出的请求由于网络延迟反而晚到怎么办呢，如果是先 DELETE 后 GET，会引起类似 RAW 的问题，所以不是所有的请求都可以放入 pipeline 的

**只有幂等的请求才可以被管道化，比如GET和HEAD**，POST和PUT不应该被管道化。幂等保证在pipeline中的所有请求可以不必关心发送次序和到达服务器后执行的次序，即使多次请求，返回的结果一直是一样的

### HTTP/1.1的性能

关于 HTTPS 的内容在网络安全部分会给出
...
* Cons
  * 请求-应答机制引起的响应队头阻塞（Head-of-Line Blocking）：在HTTP/1.1中，每个请求都需要单独建立维护连接，当一个响应阻塞时，后续响应也会被阻塞，导致性能下降
...
3. 请求-应答模式引起队头阻塞问题。之所以要采取请求-应答机制的原因主要是因为 HTTP 协议本身设计的原因，没有办法说明哪个响应是发给哪个请求的，所以只能根据请求的顺序来一一响应

4. 冗长的报头以文本形式发送，难以压缩：由于 HTTP 是无状态的，所以所有信息都要在报头中携带，特别是当使用 cookie 时，报头更大了
...
HTTP/2是基于HTTPS的，所以HTTP/2保证了安全性。并且为了推广 HTTP/2，HTTP/2 是兼容 HTTP/1.1 和 1.0 的

针对上面所述的 HTTP/1.1 的性能瓶颈，HTTP/2做出了如下的设计

### 头部压缩的 HPACK 算法

https://halfrost.com/http2-header-compression/

HTTP/2 的 HPACK 算法的 RFC7541 标准：https://httpwg.org/specs/rfc7541.html

前面说过，HTTP/1.1 针对响应报文的 body 部分可以通过 Content-Encoding 字段来规定使用哪种压缩方式，**HTTP/2 采用了类似的压缩算法来压缩报文的 body 部分**

但是针对庞大的 header 部分却仍然要通过文本形式（UTF-8码）表示，没有压缩的方式

由于以下场景的存在，因此 header 很有压缩的必要

* Header 含很多固定的字段，比如 Cookie、User Agent、Accept 等，这些字段加起来也高达几百字节甚至上千字节
* 大量的请求和响应的报文里有很多字段值都是重复的，这样会使得大量传输带宽被这些冗余的数据占用了，所以有必须要**避免重复性**

HTTP/2 使用独创的 HPACK 算法来压缩 header，HPACK 算法主要包含三个组成部分：静态字典、动态字典和 Huffman 编码（压缩算法）

* Client 和 Server 双方会建立起字典，类似于数据库 tuple 的 dictionary encoding 压缩方法，即用较少数据量的 key 来索引较大数据量的 value，在传输的时候都使用这个 key 来通信
* 另外再用 Huffman 编码来压缩 value 本身
* 两者结合可达到 50%~90% 的高压缩率
...
* 静态表 static table
  ...
  HTTP/2 为**高频**出现在头部的字符串和字段预先建立了一张下面这样的静态表，Index是从 1 ～ 61，具体完整的表可以查询 RFC7541 标准
  ...
  上面的 Header Name 是响应报头的字段名，Header Value 则是其值，其中笔者用 `/` 标准的不是说没有值，而是说它们是变长的、不固定的。这些 Value 都会经过 Huffman 编码后，才会被发送出去

  <img src="使用静态表的头部报文.drawio.png" width="70%">

* 动态表 dynamic table

  对于那些没有那么高频的 Header 字段来说，就需要 CS 双方在通信过程中动态地构建动态词典了。动态表跟在静态表后面，Index从62开始

  动态表越大，占用的内存也就越大。若占用了太多内存，会影响服务器性能，因此 Web 服务器都会提供类似 `http2_max_requests` 的配置，用于限制一个连接上能够传输的请求数量，避免动态表无限增大，请求数量到达上限后，就会关闭 HTTP/2 连接来释放内存

### stream 并发传输

将一个 TCP 连接切分成为多个 stream，每个 stream 都有自己的 stream id，这个 stream 可以是客户端发往服务端，也可以是服务端发往客户端

笔者感觉说白了就是通过 stream ID 机制让响应和请求可以对应起来，从而实现多个不同的 stream 的乱序并发。HTTP/1.1 没有这种对应机制，所以就只能做成响应 - 应答机制

<img src="http2多路复用.jpeg.webp">

Hierarchy：帧 frame `->` 信息 message `->` 流 stream `->` 一个 TCP 连接

* 一个 message 包含多个帧，帧就是下面说的二进制帧，这是 HTTP/2 的最小单位
* 一个 stream 里可以包含多个双向 message，由 HTTP 头部和包体构成
* 一个 TCP 连接包含一个或者多个 stream

如果 stream ID 不够了怎么办，得关闭当前的 TCP 连接，然后 CS 双方重新建立连接

### 使用二进制帧

相比于 HTTP/1.1 采用全文本传输，HTTP/2 将二进制帧作为其最小传输单位，而一个帧内则存放着多个上面说过的经过 HPACK 压缩的 header 和经过 gzip 等算法压缩的 body 部分

<img src="HTTP2的二进制帧.drawio.png" width="60%">

* 帧类型：HTTP/2 总共定义了 10 种类型的帧，一般分为数据帧和控制帧两类
* 帧类型后面的一个字节是**标志位**，可以保存 8 个标志位，用于携带简单的控制信息，比如：
  * **END_HEADERS** 表示头数据结束标志，相当于 HTTP/1 报文中的的空行 `\r\n`
  * **END_Stream** 表示单方向数据发送结束，后续不会再有数据帧
  * **PRIORITY** 表示流的优先级
* 流标识符就是 stream ID，如上所述，是可以让 stream 并发的关键
* Frame Payload 中就是经过 HPACK 压缩的 header 和经过 gzip 等算法压缩的 body 部分

### 服务器主动推送资源

CS**双方都可以建立 Stream**，因为 server 可以主动推送资源给 client。为了分开CS双方，**client 建立的 stream 必须是奇数号，而 server 建立的 stream 必须是偶数号**
...
早年为了解决 TPC 的分包和粘包问题，需要依赖应用层制定的协议。这个发展过程中就涌现出了大量的应用层协议，比如说 HTTP 或者 RPC

实际上，RPC 协议（20世纪70年代）推出的时间早于 HTTP 协议（20世纪80年代）。个人理解这是因为早期还没有大规模的互联网络，而是一些机构内的局域网，RPC 就是更是应用这种场景的，知道今天也是这样，RPC更适合于 CS 或者局域网/内网之间的通信

其实论效率而言，HTTP/2 在引入 HPACK 压缩算法等机制后效率并不会低于 RPC，但是 RPC 可能已经在公司内部使用很久了，所以替换也不需要一时半会就得提上日程

### RPC协议的实现

RPC只是一种实现，具体的一些开源 RPC 框架有 gRPC、thrift、Dubbo、Avro等，但它们都脱离不开 RPC 协议的规定

Host A 上的一个应邀想要调用 Host B 暴露出来的函数/方法时，由于不是在一个内存空间中的，自然不可能直接调用，必须要通过网络来表达调用的语义和传递参数

<img src="RPC实现.drawio.png" width="60%">

整个调用过程主要是如下几个步骤

1. 建立通信连接

   主要是通过在客户端和服务器之间建立TCP连接，远程过程调用的所有交换的数据都在这个连接里传输。连接可以是按需连接的短链接，调用结束后就断掉，也可以是长连接，多个远程过程调用共享同一个连接

1. 服务寻址
   如上所述，通信双方可能不在同一个内存空间上，所以要解决寻址的问题，即 server 服务器上的应用怎么告诉底层的RPC框架，如何连接到其特定的端口，以及方法的名称名称是什么

   可靠的寻址方式（主要是提供服务发现）是RPC的实现基石，通常情况下我们需要提供 server 的信息（主机名或IP地址）以及特定的端口，然后指定调用的方法或者函数的名称以及参数等信息，这样才能完成服务的一个调用

   如上图所示，**这些服务由一个用户层的软件层 -- 注册中心 Registry Center 来提供**，比如可以采用 Redis、Zookeeper、Eureka 等注册中心 来注册服务等等

   <img src="注册中心的功能.drawio.png" width="60%">

   * 服务提供者
     * 当提供者服务启动时，需要自动向注册中心注册服务
     * 当提供者服务停止时，需要向注册中心注销服务
     * 提供者需要定时向注册中心发送心跳 heartbeat，一段时间未收到来自提供者的心跳后，认为提供者已经停止服务，从注册中心上注销掉对应的服务
   * 调用者
     * 调用者启动时订阅注册中心的消息并从注册中心获取提供者的地址
     * 当有提供者上线或者下线时，注册中心会告知到调用者
     * 调用者下线时，取消订阅

1. 服务调用 & 网络传输：序列化 & 反序列化。这部分功能也是由注册中心中的 stub 完成的

   Server 进行本地调用，一般是通过生成代理Proxy去调用，通常会有JDK动态代理、CGLIB动态代理、Javassist生成字节码技术等，之后得到本地调用的返回值。此后序列化后交由底层的 TCP/IP 协议传输

   * 客户端存根 client stub：存放服务端地址信息和接口信息等，将客户端的请求参数数据信息打包成网络消息，再通过网络传输发送给服务端
   * 服务端存根 server stub：接收客户端发送过来的请求消息并进行解包，然后再调用本地服务进行处理

## <span id="QUIC">*QUIC*</span>

QUIC, Quick UDP Internet Connections 是由 Google 开发的一种基于 UDP 的传输层协议，旨在提高网络连接的速度和可靠性，以取代当前互联网基础设施中广泛使用的 TCP 协议。 QUIC 通过加密和多路复用技术来提供更高的安全性和更快的数据传输

要基于不可靠的 UDP 实现的可靠传输协议，那么就要在应用层下功夫，也就是要设计好协议的头部字段

## *FTP*

FTP, File Transfer Protocol 文件传输协议 是用于在网络上进行文件传输的标准协议。它允许用户通过网络从一个计算机（称为FTP服务器）向另一个计算机（称为FTP客户端）传输文件。FTP是Internet上使用最广泛的文件传输协议之一，通常用于在服务器和客户端之间进行文件共享和管理
...
TCP协议 Transmission Control Protocol：**可靠、面向连接、面向字节流**
...
* 分包 packet fragmentation
  ...
  TCP 是以段 segment 为单位发送数据的，建立 TCP 连接后有一个最大消息长度 MSS。如果应用层数据包超过 MSS 就会把应用层数据包拆分，分成两个段来发送。这个时候接收端的应用层就要拼接这两个 TCP 包，才能正确处理数据
...
<img src="应用层协议定制以应对粘包分包问题.drawio.png" width="40%">
...
2. **`server_isn` + `SYN+ACK`+ `Ack Num`**：Server也会随机初始化序列号 `srever_isn` 并填入“序列号”字段中，然后将确认应答 ACK 设置为 `client_isn+1`，最后将 `SYN` 和 `ACK` 标志位设为1后将报文发送给客户端。Server发送之后的状态变成 `SYN_RCVD`。对于客户对服务器的 SYN 的确认被装载在服务器对客户的数据中，这被称为**捎带 piggybacking**
...
RFC793提到初始化序列号 ISN, Initial Sequence Number 的随机生成算法为 `ISN = M + F(localhost, localport, remotehost, remoteport)`
...
另外从安全角度看：若只进行一次握手或两次握手，那么很容易被攻击（SYN Flood，发送大量SYN请求）。奇数次的握手把最后一次ACK丢失的成本转移给了发起通话的客户端。当然这并不意味着三次握手就可以完全避免SYN洪水攻击
...
* SYN queue/半连接队列：第一次握手后，Server收到Client发起的SYN请求，内核会为该连接维护一个半连接队列，此时服务端的状态处于 SYN_RCVD
...
建立起了连接之后，双方进行全双工通信，即数据在两个方向上能同时传递，可理解为两个方向相反的独立通道。这是由TCP的**半关闭** half-close 造成的。所谓的半关闭就是TCP提供了连接的一端在结束它的发送后还能接收来自另一端数据的能力，因此要进行4次挥手 four-way wavehand

第一次分手无论是客户端还是服务端都可以发起

* 四次挥手的过程：服务端和客户端分别需要发出FIN和ACK，也就是各两次挥手，因此加起来4次挥手
* 第三次握手后，客户端向服务端发出ACK后，客户端的状态并不是马上就变成CLOSED，而是要变成TIME_WAIT等待一段时间（即2被 MSL, Maximum Segment Lifetime 报文段最大生成时间）。这是为了最大程度上保证ACK达到了服务端，因为若ACK丢失了，服务端在发出FIN一端时间后若仍然没有收到ACK，就会进行FIN的超时重传。这就是为什么当关闭了某个端口的服务器后，不能立即重启这个端口的服务器
* 很巧合的情况下，比如服务端和客户端同时希望断开连接，也可以只有三次挥手，此时第二次挥手就是FIN+ACK

为什么要进行4次挥手？因为在任何一端收到 FIN 并发出 ACK 之后，可能还需要继续发送一些报文（全双工，发送短关闭了不影响接受端），只有当发送完毕了才会主动发起 FIN

### 4次挥手中出现报文丢失的情况

### `TIME_WAIT` 不能够及时启动的问题

通过设置 `setsocket()` 的 SO_REUSEADDR 选项来允许地址重用。设置了这个标志，那么即使之前的连接仍在占用该地址，新的监听器也可以绑定到该地址
...
以太网的交换节点经历了总线拓扑（同轴电缆）`->` 集线器 hub `->` `->` 网桥 `->` 交换机的历程
...
网桥 network bridge 又称为桥接器，可以理解为早期的低端交换机。现代网络中大部分使用的链路层网络设备都是交换机

<img src="网桥和交换机.drawio.png" width="70%">

早期的局域网使用网桥来连接不同的物理介质，如连接以太网和令牌环网。网桥通常连接的就是两个网络（因此是2个端口），而交换机的功能更加强大，能连接多大上百个不同的网络

网桥是一种基于软件的设备，通过CAM表学习和过滤 MAC 地址来实现数据帧的转发
...
HTTPS是在在HTTP应用层和传输层之间加了一层SSL, Secure Sockets Layer/TLS, Transport Layer Security 软件层
...
2. 双方协商生产会话秘钥
3. 双方采用会话秘钥进行加密通信（HTTP）

其中前两步是 SSL/TLS 的建立/握手阶段。SSL/TLS 的握手阶段涉及4次通信，且根据使用的密钥交换算法不同，TLS 的握手流程也不同。常用的密钥交换方式有下面这几种
