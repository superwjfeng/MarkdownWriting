...
* 自定义类型右值：将亡值 expiring value/xvalue，因为将亡值可以是从左值转换来的，所以既可以算作左值，也可以算作右值
...
* `static_cast<type &&>`
* `std::move`：但是右值引用可以引用move以后的左值，`std::move` 定义在 `<utility>` 中。但是**右值引用的结果仍是左值**。注意：如果类中没有实现有效的移动构造，则执行的依然是拷贝
