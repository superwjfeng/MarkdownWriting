...
由于历史的原因，Stroustrup在设计模板的时候没有引入一个新的关键字，用的仍然是 class，很多旧的编译器只支持 class
...
萃取有两种：类型萃取（type traits）和迭代器萃取（iterator traits），这是一种元编程技术，用于获取和描述类型或迭代器的特性。这在泛型编程和模板元编程中非常有用，以便在编译时确定对象或类型的特性。

1. **类型萃取（Type Traits）**：类型萃取是用于获取和描述类型特性的技术。在C++标准库中，有许多类型萃取工具，例如`std::is_integral`、`std::is_floating_point`、`std::is_pointer`等，它们用于检查给定类型是否具有某些特性，例如是否为整数、浮点数、指针等。这些工具可以在编译时根据类型信息做出决策，以确保类型安全性和性能。
2. **迭代器萃取（Iterator Traits）**：迭代器萃取是用于获取和描述迭代器的特性的技术。在泛型编程中，你经常需要了解迭代器的特性，以确定它支持哪些操作，例如遍历、递增、递减等。通过迭代器萃取，你可以在编译时确定迭代器的特性，从而编写通用的算法，这些算法可以与不同类型的迭代器一起使用。

这些技术通常依赖于C++模板元编程和SFINAE（Substitution Failure Is Not An Error）原则，它们允许在编译时根据类型特性进行条件编译和编程决策，以确保代码的正确性和性能。这是C++中非常强大的编程技术之一，可以用于创建高度通用的库和算法。
