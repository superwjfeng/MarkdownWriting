...
右值引用只能引用右值，不能引用左值。可以将泛左值转换为将亡值

* `static_cast<type &&>`
* `std::move`：但是右值引用可以引用move以后的左值，`std::move` 定义在 `<utility>` 中。注意：如果类中没有实现有效的移动构造，则执行的依然是拷贝
* C++17引入了临时量实质化，具体可以看 *现代Cpp.md*
...
底层const和顶层const是C++ Primer引入的概念。可以这么理解：顶层const是对象本身，而底层const则是从底层来看是某个对象的地址
