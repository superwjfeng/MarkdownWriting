...
在编译期间，根据实际传入的 expr 和预设的 ParamType 的不同类型的排列组合来推导模板类型 T

ParamType = 饰词 + T，饰词有 cosnt、volatile、`&`、`&&`、`*` 等

### ParamType 既非指针也非引用

此时就是**按值传递**，也就是说无论传入的是什么，param 都会是它的一个副本。所以下面的性质是很好理解的，反正函数模板内部得到的都是一个实参拷贝，是不会拷贝它的性质的（原理就是所处区域不一样，一个栈区的拷贝常量区里的内存，不会拷贝它常量区的性质），所以对它做什么都不会影响原来的值

1. **忽略 expr 的引用性 reference-ness、顶层常量性 top-level constness、volatile性**。注意一个点：右值引用得到的仍然是一个左值引用，所以当expr是一个右值引用时候的，它等同于一个左值引用，所以引用性同样会被忽略
...
**当 `T&&` 类型出现在模板函数中**，且 `T` 是由模板参数推导得出时（意思是**只有纯净的 `T&&` 是万能引用**，`const T&&` 是一个右值引用），此时称为万能引用

* 如果传入的 expr 是一个**右值**，那么 T 和 ParamType 将被推导为该右值的**实际类型**（或者说运用 ParamType 是非万能引用的指针或引用时的规则，即去掉 expr 的引用性，然后拼接上 `&&`）。**此时 T 是一个左值！**
* 如果传入的 expr 是一个**左值**，那么 T 和 ParamType 将被推导为该左值的类型加上左值引用。**这是在模板类型推导中，T被推导为引用类别的唯一场景**。这种情况被称为引用折叠 reference collapse，相当于 `int & + int && -> int &`，相当于三个引用折叠成了一个引用
* **换句话说，当将一个左值传递给一个接受右值引用的模板函数时，推导出来的是一个左值引用类型，因此函数可以接收左值作为其参数。这样的设计使得模板函数能够同时接受左值和右值参数，为泛型编程提供了极大的便利**
...
f(x);   // 左值x，T 被推导为 int&，引用折叠
f(cx);  // 左值cx，T 被推导为 const int&，引用折叠
f(rx);  // 左值rx，T 被推导为 const int&，引用折叠
f(20);  // 右值，T 被被推导为 int&&
```

这里有一个很重要的点要解释一下：**当在函数体内给一个对象起名时（即为其创建一个变量名），这个对象被视为左值，无论它在传递给函数之前是左值还是右值**。这一点尤其重要，因为它决定了可以对该对象执行哪些操作，例如赋值或取地址，这对我们后面理解完美转发有重要作用
...
auto 类型推导和模板类型推导的规则基本上完全一致，它们之间可以建立起一一映射的关系，它们之间也确实存在双向的算法变换
...
采用 auto 进行变量声明时，**auto 的型别饰词取代了ParamType，即 auto 是要推导的对象**，所以此时也是根据 auto 的型别饰词分成三种类型来讨论。并且同样的，数组和函数的退化情况也同样适用于auto
...
此外，C++14 中又引入了一种**auto可以用作模板类型推导**的方式，需要配合 decltype 一块使用，具体见下一条条款
...
注意：`int a` 是一个变量，`decltype(a)` 得到的类型是 int。若想要用它的表达式属性，可以用 `()` 括起来，`decltype((a))` 返回的是 `int &`，这如果是在函数内部使用将是灾难性的，因为返回了一个右值的引用，是一种未定义行为

decltype 并不会真的去计算表达式的值，编译器只是会分析表达式并得到类型
...
C++11 中 decltype 的主要用途大概就是用来声明那些返回值型别依赖于形参类型的函数模板

考虑写一个返回容器 `[]` 调用的函数模板，虽然大多数时候 `operator[]` 都会返回和容器数据类型T相同的 `T&`，但是若保存的数据类型是 bool 就不行了，比如 `std::vector<bool>` 返回的是 `std::vector<bool>::reference` 类型（至于为什么这么处理在[条款 6](#条款 6：auto 推导若非己愿，使用显式类型初始化惯用法) 中有说明）。因此我们要对其进行 delctype 推导

* C++11 的写法： trailing return，此时 auto 是一个返回值的置位符
...
* C++14 的写法，因为 C++14 将自动推导返回值推广到了包含多语句的一切 lambda 表达式和函数式，所以此时可以把尾置返回去掉了

  * 错误的写法：可以直接写 auto，但是此时要注意 **auto 走的是模板推导**，此时其饰词既不是引用，也不是指针，所以当接受的 expr 是引用的时候，输入的引用性会被忽略，所以 auto 得到的不是一个引用，而是一个值拷贝。此时相当于是把5赋值给一个返回值（即一个右值），因此无法通过编译，也就无法实现修改
  ...
  * 正确的写法：使用 `decltype(auto)`，它的作用简单来说就是**告诉编译器用 decltype 的推导表达式规则来推导 auto，而不是走 auto 的模板推导**。此时等价于 C++11 的 auto 占位符写法，意思是保存引用性质，否则引用性会被模板脱掉
...
C ++17 中和 auto 一样，`decltype(auto)` 也能作为非类型模板形参的占位符，其推导规则和上面介绍的保持一致
...
**`std::forward<T>` 的本质是有条件的 move，只有当模板参数 T 用右值初始化时才转换为右值**，而 `std::move` 的本质就是无论左值还是右值统统转为右值
...
第二种含义是万能引用 universal reference（也称为通用引用和转发引用 forwarding references），出现在模板和 auto 这两种类型推导的场景中
...
要求是 `Args&&...` + 类型推导。这是一种**可以接收任意数量和类型的参数**的模板，并将它们转发给其他函数，同时保持原始参数的值类别（左值或右值）

这项技术非常有用，尤其是在构造函数、赋值运算符和各种通用工具类（如标准库中的 `std::tuple` 或 `std::pair`）中，它们需要完美转发任意数量和类型的参数

以下是一个使用万能引用和可变参数模板的例子，`forwarder` 函数模板利用了可变参数模板和万能引用的组合来接收任意数量的参数。然后，它使用 `std::forward` 来保证这些参数以其原始的值类别（左值或右值）传递给 `process` 函数。由于我们使用了参数包展开语法（`args...`），所以无论传递给 `forwarder` 多少参数，它们都会被展开并分别转发

```c++
#include <iostream>
#include <utility> // for std::forward

// 这个函数负责接收任何数量的参数，并将它们转发给另一个函数
...
void process(double x, double y) {
    std::cout << "处理两个双精度浮点数: " << x << ", " << y << std::endl;
}

int main() {
    int a = 10;

    // 调用 forwarder 并传递不同数量和类型的参数
    forwarder(a);         // 'a' 是左值，完美转发为左值
    forwarder(20);        // '20' 是右值，完美转发为右值
    forwarder(3.14, 2.71); // '3.14' 和 '2.71' 都是右值，完美转发为右值

    return 0;
}
```

和 push_back 不同，下面 emplace_back 的参数包 `Args&&...` 和 vector 的 T 没关系，所以 emplace_back 是可变参数模板的万能引用
...
auto 万能引用在 C++14 中大量出现，因为 C++14 支持 lambda 可以声明 `auto&&` 形参
...
[vector中emplace_back和push_back详解，源码解读_c++的vector pushback 的源码实现-CSDN博客](https://blog.csdn.net/wang664626482/article/details/123348383)
...
template<class... _Valty>
	...
	template<class _Objty,
		...
		// 最终传入的参数类型在这里使用，根据完美转发规则，传入的是什么类型参数，然后匹配其对象对应的构造函数进行对象构造。如果是一系列参数，就调用自定义的构造函数，如果是左值，就调用拷贝构造，如果是右值，就调用移动构造。若没有移动构造，就调用拷贝构造
...
SFINAE 的意思是：当使用模板函数时，**如果模板有特化过**，编译器会根据传入的参数来推导适配最合适的模板函数。而在某些情况下，推导过程会发现某一个或者某几个模板函数推导起来其实是无法编译通过的，**但只要有一个可以正确推导并编译出来**，则那些推导得到的可能产生编译错误的模板函数就并不会引发编译错误，也就是匹配失败不是错误
...
```C++
// since C++11
// 通用模板定义 primary template
template<bool B, class T = void> struct enable_if {};
// 偏特化模板 partial specialization
...
* 在模板的第一个参数 condition 为真的时候，由于偏特化机制，第二个结构体模板明显是一个更好的匹配，所以 `std::enable_if<>::type` 就是有效的。**根据 SFINAE 规则** ，第一个通用模板定义虽然只是不匹配，不会报错
...
* 校验函数模板的参数
  ...
  [C++ 类型萃取 | 编程指北](https://csguide.cn/cpp/modern_cpp/type_traits.html#什么是-type-traits)
...
* 类型控制函数的返回类型（**enable_if 的第二个模板参数此时才会有用**）
...
* 模板类型偏特化：可以利用 `std::enable_if` 的特性根据模板参数的不同特性进行不同的类型选择

  [【C++编程】std::enable_if 和 SFINAE - 苏格拉底的落泪 - 博客园](https://www.cnblogs.com/sunbines/p/15260214.html)
