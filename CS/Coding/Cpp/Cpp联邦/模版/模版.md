...
注意⚠️：类的默认模版参数的用法和函数缺省值一样，只能是从右往左写
...
其实traits的概念很好理解，不要被它的中文翻译“萃取”吓到了，**traits就是在告诉我们一个模板的特征，萃取的操作对象是模版的模版参数**。这是一种元编程技术，用于获取和描述类型或迭代器的特性。这在泛型编程和TMP中非常有用，以便在编译时确定对象或类型的特性
...
* 类型萃取 type traits：发源于boost库，这种思想被程序员所接受
