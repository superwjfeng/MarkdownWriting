...
* C++98 的左值 lvalue 一般是指一个指向特定内存的具有名称的值（具名对象），它有一个相对稳定的内存地址，且生命周期较长
  ...
  **左值的特点是可以取地址**，一般是一个变量或者返回引用的表达式。**除了 const **不能被赋值外其他可以放到赋值符号左边。因此为了适应 const 的变化，C 标准新增了一个术语：**可修改的左值 modifiable lavalue**，用于标识可修改的对象。也可以称为对象定位值 object locator value
  ...
  字符串常量是一个不可修改的左值，比方说下面的 p 是一个指向常量区字符串首字符的指针，常量区对于完全一样的字符串只会开一个
...
const 左值可以引用右值这个特点在引用传参的时候被用到了，即引用传参时既可以接收左值也可以接收右值，事实上 **const 左值引用可以接受任何值**
...
* 除了 `.* :: sizeof ?: .` 5个运算符外其他的运算符都可以被重载，即
  * 数学运算符
  * 关系运算符
  * 逻辑运算符
  * 单目运算符
...
* 当一个重载的运算符是成员对象时，this 绑定到左侧运算对象

## *运算符重载的实现方式*

### 类内重载（成员函数）

当运算符仅涉及一个操作数，或者当二元运算符的**第一个操作数必须是类的实例时**，通常会选择类内重载。实现为成员函数的运算符可以直接访问类的私有和保护成员
...
- 赋值 `=`、下标 `[]`、调用 `()` 和成员访问箭头 `->` 运算符必须是成员，因为它们都要访问 this
- 复合赋值运算符一般来说应该是成员，但这并非必须的，这一点与赋值运算符略有不同
- 改变对象状态的运算符或者与给定类型密切相关的运算符，如递增 `++` 、递减 `--` 和解引用运算符，通常应该是成员
- 数学、比较运算符也可以实现第一个参数一定是当前类对象的版本
- 当想要链式调用时

### 类外重载（非成员函数）

如果一个运算符预期其两个操作数在语义上处于平等地位，特别是当操作数可以是不同类型时，将该运算符重载为非成员函数更合适。这也适用于内置类型无法作为成员函数第一个操作数的情况
...
MyClass operator+(const MyClass& lhs, const MyClass& rhs) {
    MyClass result = lhs; // 可能需要复制构造函数
    result += rhs;        // 调用之前定义的 operator+=
    return result;
}
```
...
- 具有**对称性**的运算符可能转换任意一端的运算对象，例如算术、相等性、关系和位运算符等，因此它们通常应该是普通的非成员函数
  - 数学运算符（如 `operator+`, `operator-`, `operator*`, `operator/`）
  - 比较运算符（如 `operator==`, `operator<`, `operator>`, 等）

### 类外友元

当需要在类外部定义一个函数，但这个函数又必须访问类的私有或保护成员时，可以将该函数声明为类的友元

友元函数并不是类的成员函数，但它们有权访问类的私有和保护成员
...
- 当函数需要访问类的非公开成员，且不能或不应该作为成员函数实现时
- 对于需要对左侧和右侧操作数进行相等处理的运算符，如流插入和提取运算符
- 当运算符的参数至少有一个参数是用户定义的类型，并且需要与其他类型进行互操作时

## *实现 Tips*

### 实现链式调用

链式调用是一种编程模式，允许在单个语句中连续执行多个方法调用。在C++中，运算符重载可以实现这种模式，通常通过返回对象的引用或指针来完成

为了支持链式调用，**运算符必须返回一个对当前对象的引用 `*this`，使得返回值可以作为下一个调用的操作数**。在[赋值运算符](#赋值运算符重载 Assignment Operator Overloading)（`operator=`）和流插入运算符（`operator<<`）我们都说明了这一点
...
* 一个类需要重载哪些运算符要看哪些运算符对这个类型有意义，比如 Date 类除法和乘法就没有意义
* 前置 `++` 和后置 `++` 重载：特殊处理，使用函数重载，**默认无参为前置，有参为后置**
    ...
    ```
    复用 `+=`，因为前置 `++` 会直接改变 `*this`，因此可以传引用返回，而后置 `++` 是先用再 `++`，因此要保留一个`*this` 的备份 tmp，所以要传值返回 tmp

### 逻辑运算符性质的“保留”

重载 `&`、`|`、`&&`、`||` 的时候要注意保留 `& |` 的运算对象求值顺序，以及保留 `&& ||` 的短路性质

逻辑运算符的短路特性意味着：对于`&&`，只有当第一个操作数为`true`时才评估第二个操作数；对于`||`，只有当第一个操作数为`false`时才评估第二个操作数。但是，在**重载的逻辑运算符中，所有参数都必须在函数调用之前被评估，因此无法保持短路行为**

下面我们通过定义一种手动控制的机制来模拟短路行为，但记住这并不是真正的短路，因为参数仍然会全部被评估
...
public:
    ...
    explicit LogicalClass(bool v) : value(v) {}

    // 这个函数用以模拟逻辑与的短路行为
    LogicalClass operator&&(const LogicalClass& other) const {
        if (!value)
            return *this; // 如果当前对象为false，则直接返回当前对象
        else
            return other; // 否则返回第二个操作数的求值结果
    }

    // 这个函数用以模拟逻辑或的短路行为
    LogicalClass operator||(const LogicalClass& other) const {
        if (value)
            return *this; // 如果当前对象为true，则直接返回当前对象
        else
            return other; // 否则返回第二个操作数的求值结果
    }
};
...
int main() {
    LogicalClass a(true), b(false);
    auto result = a && b; // 通常应该只评估a就足够了，但实际上a和b都会被评估
    auto result2 = a || b; // 通常应该只评估a就足够了，但实际上a和b都会被评估
...
与 iostream 标准库兼容的 IO 运算符必须是普通的非成员函数。因为 iostream 标准库中第一个形参必须是 iostream 对象，若定义为成员函数则第一个对象是隐含的 `*this` 指针，因此一定要定义为非成员函数

若定义为类的非成员函数，则 IO 运算符重载娶不到类的私有数据，IO 运算符通常都需要读写类的非公有数据成员，所以 IO 运算符一般都会被声明为友元来获取权限

例子：回顾 C++ IO 流的内容，`<<` 流输出运算和 `>>` 流输入，因为 C++ 库里写好了运算符重载。`cout` 对象能做到自动识别类型，这是因为所有内置类型构成了函数重载

为了支持对自定义类型 Date 类的流输入和流输出，我们需要分别为 `istream` 和 `ostream` 类重载针对Date类的 `>>` 和 `<<` 运算符重载
...
* 在 Date 类外定义的话又会产生取不到私有的 `_year` 等Date类成员的问题
...
Java 对设计模式的要求是比较高的，因为 Java 本身的库设计就用到了很多设计模式。而 C++ 则没有很普遍使用设计模式。设计模式是有益的工具，可以提高代码的可读性、可维护性和灵活性。然而过度使用设计模式可能导致复杂性增加、过度工程化、不必要的复用、过度抽象化和性能损失等劣势。在应用设计模式时，应根据具体的问题和需求进行合理的权衡，避免滥用和不必要的复杂性
...
单例模式在何时创建这个唯一的对象有区别，即饿汉模式和懒汉模式

### 饿汉模式

饿汉模式 eager Initialization：不管将来用不用，程序启动时就创建一个唯一的实例对象

**构造私有、禁止拷贝，在类外定义全局 static 成员并用 new 实例赋值给它用来保存唯一一个实例，类内必须声明 static 成员以让构造取到、提供一个 Get  方法以取到实例**
...
public:
    static Singleton* GetInstance() {
        if (inst_ == nullptr) { // 第一层if
            pthread_mutex_lock(&mutex);
            if (inst_ == nullptr) { // 第二层if，第一次调用的时候创建对象
                inst_ = new Singleton();
            }
        }
        return inst_;
...
private:
    Singleton() = default;
    ~Singleton() = default;
    Singleton(const Singeleton &) = delete;
...
pthread_mutex_t Singelton::mutex_ = PTHREAD_MUTEX_INITIALIZER;
```

* 第一层 if：为了避免在实例已经被创建的情况下再次进入互斥锁的区域。如果实例已经被创建，那么直接返回已存在的实例，避免重复创建
* 第二层 if：在获得锁之后，再次检查实例是否为 nullptr。这是因为有可能在等待锁的过程中，其他线程已经创建了实例。如果实例尚未创建，则在这个临界区内部创建实例。这样可以确保只有一个线程在创建实例，避免了多次创建
...
public:
    static Singleton* GetInstance() {
        if (inst_ == nullptr) { // 第一层检查
            std::lock_guard<std::mutex> lock(mutex_); // 使用 RAII 锁管理
            if (inst_ == nullptr) { // 第二层检查
                inst_ = new Singleton();
            }
        }
        return inst_;
...
public:
    static Singleton* GetInstance() {
        auto *p = inst_.load(std::memory_order_acquire);
        if (inst_ == nullptr) {
            std::lock_guard<std::mutex> lock(mutex_);
            if (!(p = instance.load(std::memory_load_relaxed))) {
                p = new Singleton();
                inst_.store(p, std::memory_order_relase)
            }
        }
        return inst_;
    }
...
其中Person被称为父类/基类 Base class，而Student被称为子类/派生类 Derived class，Person 前的访问限定符规定了派生类的继承方式
...
* 当省略继承方式时，**class 的默认继承方式是 private**，而 struct 的默认继承方式是 public
* 继承访问限定符是一个很冗余的设定，**实际运用中一般只会使用 public 继承，然后通过类成员是用public还是protected修饰来区分类内外是否可以访问**
...
* 若一个虚函数不想被重写，可以被定义为 final。注意：此时应该把 virtual 加上，这样如果派生类不当心重写 override 会报编译时错误，把virtual去掉了形成隐藏 hide，反而不符合设计意图
...
* `override`关键字仅需要出现在成员函数的声明中，通常这些声明位于头文件（`.h`或`.hpp`文件）中。对**应的函数定义可以位于源文件（`.cpp`文件）中，而且在源文件中实现时不需要（也不应该）再次使用`override`关键字**。这是因为`override`关键字的目的是为了告知编译器和阅读头文件的人员该函数是重写的，并让编译器进行检查。一旦声明中正确使用了`override`，在定义中就无需再使用
...
注意：上面的例子中派生类有基类没有的独立的虚函数 `virtual void func3();`，该虚函数的函数指针也会被放进虚表里，但 VS 2019 中的监视窗口是看不到这个函数指针的，这应该是一个 bug；反过来若基类有派生类没有就看得到。可以通过设计打印函数来观察
...
* 向下转型 downcasting：基类对象指针或引用转换为派生类对象或引用。注意：这里基类对象指针指的是该指针被声明为基类，但它实际指向的是什么我们不得而知，比如 `Base* base = new Derived` 指向的是一个派生类，而 `Base* base = new Base` 指向的则是一个基类
  ...
  * 这时候只有使用 `dynamic_cast` 才是安全的。安全是什么意思？
    1. 如果基类指针是指向派生类型的对象，那么可以转换是安全的
    2. 如果基类指针是指向基类类型的对象，那么不能转换，转换表达式返回nullptr
    3. 若转型失败且要转换的目标类型是引用类型，则它抛出与类型 `std::bad_cast` 的处理块匹配的异常

```c++
// 这是第一种情况，待转指针虽然声明为 Base，但是实际指向 Derived
Base* base = new Derived;
if(Derived *der= dynamic_cast<Derived*>(base)) {
    std::cout << "Dynamic cast successful." << std::endl;
}
// 这是第二种情况，待转指针声明为 Base，实际也指向 Base，此时是不能转换的，有越界风险
Base * base1 = new Base;
...
* 什么是重载、重写/覆盖、重定义/隐藏？
  ...
  * **重写 override** 是针对基类中的虚函数，派生类提供了一个具有相同签名的新实现。使用`virtual`和`override`关键字可以明确表示函数的重写关系，并启用多态行为
  * **隐藏 hide** 是指派生类提供了一个新的实现，这个实现可能因为签名不同或者基类函数不是虚函数而隐藏了基类中的版本。隐藏通常是不希望看到的，因为它可能导致意外的行为，特别是在维护大型代码库时。通常最好避免隐藏基类成员，除非有明确的目的
