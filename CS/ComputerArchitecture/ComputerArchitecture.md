...
* MIPS（Microprocessor without Interlocked Pipeline Stages）是一种经典的 RISC（Reduced Instruction Set Computer）架构。它注重简单指令集和高性能设计。MIPS的中文直译为无内部互锁流水级的微处理器，其机制是**尽量利用软件办法避免流水线中的数据相关问题**
...
* RISC-V 是一种开源 RISC（Reduced Instruction Set Computer）架构，具有可伸缩性和可定制性
...
* Intel x86 是一种复杂指令集计算机（CISC）架构，由英特尔推出。它广泛应用于个人电脑领域
