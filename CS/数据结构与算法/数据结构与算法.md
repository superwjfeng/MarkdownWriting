...
* 双向带头循环链表 Circular doubly linked list with dummy node：结构最复杂的链表，一般用在单独存储数据，在实际中所使用的链表数据结构，都是带头双向循环链表
    ...
    <img src="Circular_doubly_linked_list.png">
...
* 假设用平衡树：内存中存了一棵平衡树的数据结构，每个节点以KV形式存了关键字+相应数据在磁盘上的位置。要进行高度次IO，即***logN***次，在内存中是完全可以接受的，每一次的层跳转/孩子节点跳转都是一次硬盘的IO，这么多次的磁盘IO是非常慢的
* 哈希的复杂度是***O(1)***，但是若在磁盘中对大体量数据映射哈希表，那么哈希冲突会很严重
...
B+树是B树的改进，B树系列里实际用的最多的是B+树。它对B树做出了如下的优化
