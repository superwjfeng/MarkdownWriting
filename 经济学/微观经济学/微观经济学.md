...
与完全竞争市场不同，完全竞争市场中的价格必须满足等与生产边界条件，但垄断企业可以控制市场价格，因此此时的价格是 $P(Q)$
...
* Marginal cost 或者统称为social marginal cost 由 private marginal costs 和 external marginal costs 组成
  $$
  MC(Q)=MC_{Pr}(Q)+MC_{Ex}(Q)
  $$

* Marginal benefits comprise private and external marginal benefits 其实就是 willingness to pay
  $$
...
Public good: a good that is non-rival and non-excludable with respect to consumption

* Non-rivalry: Consumption by one individual does not diminish the consumption possibilities of other individuals，一个消费者对公共品的消费不会减少其他消费者的消费机会，比如供电
* Non-excludability: It is effectively impossible to prevent any individual from consuming a public good，所有人都可以消费

实际上可以通过上面两个性质对商品进行分类
