...
[号称最难理解的设计模式，只需看这篇解析——桥接模式-CSDN博客](https://fengjungle.blog.csdn.net/article/details/102694306)

桥接模式 bridge pattern 是一种用于将抽象部分与实现部分解耦的设计模式

桥接模式将两个独立变化的维度设计成两个独立的继承等级结构（而不会将两者耦合在一起形成多层继承结构），在抽象层将二者建立起一个抽象关联，该关联关系类似一座桥，将两个独立的等级结构连接起来，所以叫做桥接模式

## *观察者/订阅模式*

[“牵一发而动全身”——我用观察者模式简单模拟吃鸡_设计模式如何解决牵一发动全身的问题-CSDN博客](https://blog.csdn.net/sinat_21107433/article/details/102927937)
